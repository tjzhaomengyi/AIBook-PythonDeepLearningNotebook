...
def gradient_ascent_step(image, filter_index, learning_rate=0.01):
    ...
    #todo:grads = tf.math.l2_normalize(grads) #应用梯度规范化技巧
...
#创建特征提取器模型，返回某一层的输出
layer_name = "block1_conv2" #这个是xception模型中的任意一层block3_sepconv1
...
#随机生成一个图片，然后进行可视化滤波器
gen_filter = generate_filter_pattern(filter_index=2)
plt.imshow(deprocess_image(gen_filter))
plt.show()
plt.savefig("filter_visual.png")